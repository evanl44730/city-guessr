...
interface GuessEntry {
    ...
    distance: number;
    direction: string;
    angle: number; // in degrees for radar positioning
}
...
const DIFFICULTY_CONFIG: Record<Difficulty, { label: string; icon: React.ReactNode; minPop: number; maxAttempts: number; color: string; desc: string }> = {
    easy: { label: 'Facile', icon: <Shield className="w-7 h-7" />, minPop: 100000, maxAttempts: 8, color: 'emerald', desc: 'Grandes villes · 8 essais' },
    medium: { label: 'Moyen', icon: <Target className="w-7 h-7" />, minPop: 20000, maxAttempts: 6, color: 'amber', desc: 'Villes moyennes · 6 essais' },
...
function calcAngle(from: { lat: number; lng: number }, to: { lat: number; lng: number }): number {
...
export default function RadarGame({ citiesData, onBack }: RadarGameProps) {
    ...
    const handleGuess = useCallback((city: CityData) => {
        ...
        // Angle from TARGET to GUESS — so the blip shows where the guess is relative to the target (center)
    ...
    }, [targetCity, guesses, maxAttempts, gamePhase]);
    ...
    const renderRadar = () => {
        ...
        return (
            <svg viewBox="0 0 320 320" className="w-56 h-56 md:w-72 md:h-72">
                {/* Background */}
                <circle cx={cx} cy={cy} r={radarRadius} fill="#0a1628" stroke="#22c55e" strokeWidth="1" strokeOpacity="0.3" />
                
                {/* Range rings */}
                {[0.25, 0.5, 0.75, 1].map(ratio => (
                    <circle key={ratio} cx={cx} cy={cy} r={radarRadius * ratio} fill="none" stroke="#22c55e" strokeWidth="0.5" strokeOpacity="0.15" />
                ))}

                {/* Cross lines */}
                <line x1={cx - radarRadius} y1={cy} x2={cx + radarRadius} y2={cy} stroke="#22c55e" strokeWidth="0.5" strokeOpacity="0.15" />
                <line x1={cx} y1={cy - radarRadius} x2={cx} y2={cy + radarRadius} stroke="#22c55e" strokeWidth="0.5" strokeOpacity="0.15" />
                ...
                {gamePhase === 'playing' && (
                    <line
                        ...
                        stroke="#22c55e" strokeWidth="2" strokeOpacity="0.6"
                    />
                )}

                {/* Sweep cone (gradient effect) */}
                {gamePhase === 'playing' && (
                    <path
                        d={`M ${cx} ${cy} L ${cx + radarRadius * Math.sin((sweepAngle - 30) * Math.PI / 180)} ${cy - radarRadius * Math.cos((sweepAngle - 30) * Math.PI / 180)} A ${radarRadius} ${radarRadius} 0 0 1 ${cx + radarRadius * Math.sin(sweepAngle * Math.PI / 180)} ${cy - radarRadius * Math.cos(sweepAngle * Math.PI / 180)} Z`}
                        fill="url(#sweepGrad)"
                        opacity="0.3"
                    />
                )}

                {/* Gradient def for sweep */}
                <defs>
                ...
                {/* Cardinal labels */}
                <text x={cx} y={cy - radarRadius - 6} textAnchor="middle" fill="#22c55e" fontSize="10" fontWeight="bold" opacity="0.5">N</text>
                <text x={cx} y={cy + radarRadius + 14} textAnchor="middle" fill="#22c55e" fontSize="10" fontWeight="bold" opacity="0.5">S</text>
                <text x={cx + radarRadius + 8} y={cy + 4} textAnchor="start" fill="#22c55e" fontSize="10" fontWeight="bold" opacity="0.5">E</text>
                <text x={cx - radarRadius - 8} y={cy + 4} textAnchor="end" fill="#22c55e" fontSize="10" fontWeight="bold" opacity="0.5">O</text>
                ...
                {/* Guess blips */}
                {guesses.map((g, i) => {
                    const dist = Math.min(g.distance / maxDist, 1);
                    const r = dist * radarRadius * 0.9;
                    ...
                    return (
                        <g key={i}>
                            ...
                            {isLatest && gamePhase === 'playing' && (
                                <circle cx={bx} cy={by} r="8" fill="none" stroke="#22c55e" strokeWidth="1" opacity="0.5">
                                    <animate attributeName="r" from="4" to="14" dur="1.5s" repeatCount="indefinite" />
                                    <animate attributeName="opacity" from="0.6" to="0" dur="1.5s" repeatCount="indefinite" />
                                </circle>
                            )}
                            <circle cx={bx} cy={by} r={isLatest ? 5 : 3.5} fill="#22c55e" opacity={isLatest ? 1 : 0.5} />
                            {/* Label */}
                            <text x={bx} y={by - 8} textAnchor="middle" fill="#86efac" fontSize="7" fontWeight="600" opacity={isLatest ? 1 : 0.6}>
                                {g.city.name.length > 10 ? g.city.name.slice(0, 9) + '…' : g.city.name}
                            </text>
                        </g>
                    );
                })}
                ...
                {/* Outer ring glow */}
                <circle cx={cx} cy={cy} r={radarRadius} fill="none" stroke="#22c55e" strokeWidth="1.5" strokeOpacity="0.4" />
            </svg>
        );
    };
        ...
        <div className="absolute inset-0 z-50 flex flex-col bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 overflow-hidden font-sans">
            ...
            <div className="flex-1 flex flex-col md:flex-row items-center justify-center gap-6 px-4 pb-4 overflow-y-auto">
                ...
                <div className="flex flex-col items-center">
                    <div className="relative">
                        ...
                        {/* Won overlay on radar */}
                        {gamePhase === 'won' && (
                            <div className="absolute inset-0 flex items-center justify-center">
                                <div className="bg-green-500/20 backdrop-blur-sm rounded-full p-4 animate-in zoom-in duration-500">
            ...
            {gamePhase === 'won' && targetCity && (
                <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-500 p-4">
                    <div className="bg-slate-900 border border-green-500/20 p-8 md:p-12 rounded-3xl shadow-2xl flex flex-col items-center max-w-md w-full text-center">
                        <div className="p-4 bg-green-500/20 rounded-full mb-4">
                            <Crosshair className="w-12 h-12 text-green-400" />
                        </div>
                        <h2 className="text-3xl font-black text-white mb-2">Cible Localisée !</h2>
                        <p className="text-slate-400 mb-1">La ville était</p>
                        <p className="text-2xl font-black text-green-400 mb-4">{targetCity.name}</p>
                        <p className="text-slate-300 mb-6">
                            Trouvée en <span className="text-green-400 font-bold">{guesses.length}</span> essai{guesses.length > 1 ? 's' : ''} sur {maxAttempts}
                        </p>
                        <div className="flex gap-3 w-full">
                            <button
                                onClick={() => startGame(difficulty!)}
                                className="flex-1 flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white py-3 rounded-xl font-bold text-lg transition-all hover:scale-105 active:scale-95"
                            >
            ...
            {gamePhase === 'lost' && targetCity && (
                <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-500 p-4">
                    <div className="bg-slate-900 border border-red-500/20 p-8 md:p-12 rounded-3xl shadow-2xl flex flex-col items-center max-w-md w-full text-center">
                        ...
                        <p className="text-sm text-slate-500 mb-6">{targetCity.population ? new Intl.NumberFormat('fr-FR').format(targetCity.population) + ' habitants' : ''}</p>
                        <div className="flex gap-3 w-full">
                            <button
                                onClick={() => startGame(difficulty!)}
